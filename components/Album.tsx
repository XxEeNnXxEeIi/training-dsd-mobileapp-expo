...
export const Album: FC = () => {
    ...
    return <>
        <View className='w-full h-full bg-orange-100 space-y-2 p-2 flex flex-col'>
            <View>
                <Text className='text-3xl text-center text-gray-900 underline italic font-semibold tracking-widest leading-10 uppercase'>Album Page</Text>
            </View>
            ...
            <ScrollView>
                {
                    album.map((album, index) => (
                        <View key ={index} className = 'flex flex-col bg-orange-400 rounded-lg p-2 mb-3'>
                            <Text>UserId: {album['userid']}, Id: {album['id']}</Text>
